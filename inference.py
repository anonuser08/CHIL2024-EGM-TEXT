...
from transformers import BigBirdForMaskedLM, LongformerForMaskedLM, BigBirdTokenizer, BigBirdForQuestionAnswering, \
                        AutoModelForMaskedLM, LongformerTokenizer, AutoTokenizer, BigBirdConfig, AutoImageProcessor, \
                            ViTForMaskedImageModeling, BeitForMaskedImageModeling, LongformerConfig
import argparse
...
def get_args():
    ...
    parser.add_argument('--ratio', type = int, help = 'Please choose forecasting ratio')
...
if __name__ == '__main__':
    ...
    if args.ratio == 3:
        test = np.load(f'./data/test_data_by_placement_na_True_True_True_True_False.npy', allow_pickle=True).item()
        test_afib = np.load(f'./data/test_data_by_placement_na_True_True_True_True_True.npy', allow_pickle=True).item()
    elif args.ratio == 2:
        test = np.load(f'./data/twotoone/test_data_by_placement_na_True_True_True_True_False.npy', allow_pickle=True).item()
        test_afib = np.load(f'./data/twotoone/test_data_by_placement_na_True_True_True_True_True.npy', allow_pickle=True).item()
    elif args.ratio == 1:
        test = np.load(f'./data/onetoone/test_data_by_placement_na_True_True_True_True_False.npy', allow_pickle=True).item()
        test_afib = np.load(f'./data/onetoone/test_data_by_placement_na_True_True_True_True_True.npy', allow_pickle=True).item()
    else:
        test = np.load(f'./data/test_data_by_placement_na_True_True_True_False_False.npy', allow_pickle=True).item()
        test_afib = np.load(f'./data/test_data_by_placement_na_True_True_True_False_True.npy', allow_pickle=True).item()

    test.update(test_afib)
