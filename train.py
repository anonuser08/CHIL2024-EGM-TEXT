...
from transformers import LongformerForMaskedLM, BigBirdForMaskedLM, BigBirdTokenizer, \
                                ...
                                AutoImageProcessor, ViTForMaskedImageModeling, BeitForMaskedImageModeling, LongformerConfig
import argparse
...
from models import VITModel, TimeSeriesModel, BeITModel
...
def get_args():
    ...
    parser.add_argument('--ratio', type = int, default = None, help = 'Please choose forecasting ratio')
    parser.add_argument('--random', action = 'store_true', help = 'Please choose whether to randomly mask out the future signals')
    ...
    parser.add_argument('--norm_loss', action='store_true', help = 'Please choose whether to normalize the loss or not')
...
def main():
    args = get_args()
    directory_path = f'./runs/checkpoint/saved_best_{args.lr}_{args.batch}_{args.patience}_{args.weight_decay}_{args.model}_{args.use_ce}_{args.mask}_{args.mlm_weight}_{args.ce_weight}_{args.ratio}_{args.toy}_{args.norm_loss}_{args.TS}_{args.TA}_{args.LF}'
    ...
    print(f'What is the ratio of forecasting being used? {args.ratio}:1')
    
    if args.ratio == 3:
        train = np.load(f'./data/train_data_by_placement_na_True_True_True_True_False.npy', allow_pickle=True).item()
        val = np.load(f'./data/val_data_by_placement_na_True_True_True_True_False.npy', allow_pickle=True).item()
        train_afib = np.load(f'./data/train_data_by_placement_na_True_True_True_True_True.npy', allow_pickle=True).item()
        val_afib = np.load(f'./data/val_data_by_placement_na_True_True_True_True_True.npy', allow_pickle=True).item()
    elif args.ratio == 2:
        train = np.load(f'./data/twotoone/train_data_by_placement_na_True_True_True_True_False.npy', allow_pickle=True).item()
        val = np.load(f'./data/twotoone/val_data_by_placement_na_True_True_True_True_False.npy', allow_pickle=True).item()
        train_afib = np.load(f'./data/twotoone/train_data_by_placement_na_True_True_True_True_True.npy', allow_pickle=True).item()
        val_afib = np.load(f'./data/twotoone/val_data_by_placement_na_True_True_True_True_True.npy', allow_pickle=True).item()
    elif args.ratio == 1:
        train = np.load(f'./data/onetoone/train_data_by_placement_na_True_True_True_True_False.npy', allow_pickle=True).item()
        val = np.load(f'./data/onetoone/val_data_by_placement_na_True_True_True_True_False.npy', allow_pickle=True).item()
        train_afib = np.load(f'./data/onetoone/train_data_by_placement_na_True_True_True_True_True.npy', allow_pickle=True).item()
        val_afib = np.load(f'./data/onetoone/val_data_by_placement_na_True_True_True_True_True.npy', allow_pickle=True).item()
    else:
        train = np.load(f'./data/train_data_by_placement_na_True_True_True_False_False.npy', allow_pickle=True).item()
        val = np.load(f'./data/val_data_by_placement_na_True_True_True_False_False.npy', allow_pickle=True).item()
        train_afib = np.load(f'./data/train_data_by_placement_na_True_True_True_False_True.npy', allow_pickle=True).item()
        val_afib = np.load(f'./data/val_data_by_placement_na_True_True_True_False_True.npy', allow_pickle=True).item()

    train.update(train_afib)
    val.update(val_afib)
    ...
    if args.model == 'raw_big':
        if args.ratio == None:
            ...
            configuration = BigBirdConfig(attention_type = 'block_sparse')
    ...
    if args.model == 'beit':
        tokenizer = AutoImageProcessor.from_pretrained("microsoft/beit-base-patch16-224-pt22k")
        pt_model = BeitForMaskedImageModeling.from_pretrained("microsoft/beit-base-patch16-224-pt22k").to(device)
        args.num_patches = (pt_model.config.image_size // pt_model.config.patch_size) ** 2
        model_hidden_size = pt_model.config.hidden_size
        model = BeITModel(pt_model, model_hidden_size, 2).to(device)
    ...
    if args.model == 'vit' or args.model == 'beit':
