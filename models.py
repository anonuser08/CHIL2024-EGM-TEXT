...
class BeITModel(nn.Module):
    def __init__( self, beit_model, beit_model_hs, num_labels):
        super().__init__()

        self.beit_model = beit_model
        self.num_labels = num_labels
        self.classifier = nn.Linear(beit_model_hs, self.num_labels)
        self.loss_fct = nn.CrossEntropyLoss()
        
    def forward(self, x, y, mask):
        outputs = self.beit_model(x, bool_masked_pos = mask)
        seq_output = outputs.logits
        logits = self.classifier(seq_output[:, 0, :])
        ce_loss = self.loss_fct(logits.view(-1, self.num_labels), y.view(-1))
        loss = ce_loss
