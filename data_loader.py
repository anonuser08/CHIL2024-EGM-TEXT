...
class EGMDataset(Dataset):
    def __init__(self, data_dict, tokenizer = None, args = None):
        ...
        # 3 to 1
        if self.args.ratio == 3:
            self.future_signal_len = 3000
        # 2 to 1
        elif self.args.ratio == 2:
            self.future_signal_len = 2000
        # 1 to 1
        elif self.args.ratio == 1:
            self.future_signal_len = 1000
    ...
    def __getitem__(self, index):
        ...
        signal = sample[:1000]
        if self.args.ratio == 3:
            signal = sample[:4000] # 3:1
        elif self.args.ratio == 2:
            signal = sample[:3000] # 2:1
        elif self.args.ratio == 1:
            signal = sample[:2000] # 1:1
        key = self.keys[index]
        afib_label = key[3]
        ...
        if self.args.ratio == 3:
            mask_indices_signal_curr = np.random.choice(self.future_signal_len + self.curr_signal_len, int(self.args.mask * (self.future_signal_len + self.curr_signal_len)), replace=False)    
            mask[1:1 + self.curr_signal_len + self.future_signal_len][mask_indices_signal_curr] = 0
        elif self.args.ratio == 2:
            mask_indices_signal_curr = np.random.choice(self.future_signal_len + self.curr_signal_len, int(self.args.mask * (self.future_signal_len + self.curr_signal_len)), replace=False)    
            mask[1:1 + self.curr_signal_len + self.future_signal_len][mask_indices_signal_curr] = 0
        elif self.args.ratio == 1:
...
class EGMDataset2(Dataset):
    def __init__(self, data_dict, vocab_size = None, args = None):
        self.data = list(data_dict.values())
        self.keys = list(data_dict.keys())
        self.args = args
    ...
    def __getitem__(self, index):
        ...
        min_val, max_val = np.min(signal), np.max(signal)
        normalized_signal = (signal - min_val) / (max_val - min_val)
        quantized_signal = np.floor(normalized_signal * self.vocab_size).astype(int)
        mask = np.ones_like(quantized_signal)
        mask_indices = np.random.choice(len(quantized_signal), int(0.75 * len(quantized_signal)), replace=False)
        mask[mask_indices] = 0  # 0 is the masked token
        
        masked_sample = np.copy(quantized_signal)
        tokenized_sample = np.copy(quantized_signal)
        masked_sample[mask_indices] = 0 # masked out sample
        
        return torch.LongTensor(masked_sample), torch.LongTensor(tokenized_sample), torch.tensor(signal, dtype=torch.float32), mask_indices
    
    
    
class ECGDataset(Dataset):
    def __init__(self, X, y, tokenizer = None, args = None):
        self.args = args
        self.data = X
        self.label = y
        self.signal_size = self.args.signal_size
